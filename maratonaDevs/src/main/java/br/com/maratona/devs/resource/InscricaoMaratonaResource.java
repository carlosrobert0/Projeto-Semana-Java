...
public class InscricaoMaratonaResource {
	...
	public Response matriculaPorId(@PathParam("id") String id) {
		...
		try {
			...
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity("Erro na solicitacao").build();
		}catch (NullPointerException e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity("Erro na solicitacao...null").build();
		}
